...
export default function AdminCMS() {
    ...
    const fetchQuestions = async () => {
      ...
      try {
        ...
        if (data) {
          ...
          setGlobalAptitudeBank(dbQuestions);
        }
      } catch (err: any) {
        ...
        try {
          ...
          const formattedData = results.data
            .map((row: any) => {
              ...
              const optA = getColValue(row, "option a");
              const optB = getColValue(row, "option b");
              const optC = getColValue(row, "option c");
              const optD = getColValue(row, "option d");
  ...
  const deleteAptitudeQuestion = async (id: string) => {
    try {
      const { error } = await supabase
        ...
        .eq("id", id);
      if (error) throw error;
      setGlobalAptitudeBank(globalAptitudeBank.filter((q) => q.id !== id));
    } catch (err: any) {
  ...
  const handleCompanySubmit = (e: React.FormEvent) => {
    ...
    if (isEditingCompany) {
      ...
      const finalForm = {
        ...
        logo:
          companyForm.logo ||
          `https://ui-avatars.com/api/?name=${companyForm.name}&background=10b981&color=fff`,
      };
  ...
  const addRound = () => {
    ...
    setCompanyForm({
      ...companyForm,
      workflow: [...companyForm.workflow, newRound],
    });
  };
  const removeRound = (id: string) =>
    setCompanyForm({
      ...companyForm,
      workflow: companyForm.workflow.filter((r) => r.id !== id),
    });
  const updateRound = (id: string, updates: Partial<InterviewRound>) =>
    setCompanyForm({
      ...companyForm,
      workflow: companyForm.workflow.map((r) =>
        r.id === id ? { ...r, ...updates } : r,
      ),
    });
  ...
  const availableTopics = Array.from(
    new Set(globalAptitudeBank.map((q) => q.topic)),
  );
  const filteredAptitude = globalAptitudeBank.filter((q) =>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-500 mt-1 font-medium">
            Manage companies, workflows, and global question banks.
          </p>
        </div>
        ...
        {activeTab === "aptitude" && (
          <div className="flex gap-3">
            ...
            <button
              ...
              className="bg-emerald-500 text-white px-6 py-3 rounded-2xl font-bold hover:bg-emerald-600 transition-all shadow-lg shadow-emerald-500/20 flex items-center gap-2 active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed"
            >
              {isUploadingCsv ? (
              ...
              {isUploadingCsv ? "Uploading to DB..." : "Import Aptitude CSV"}
            </button>
          </div>
        )}
      </div>
      ...
      <AnimatePresence mode="wait">
              ...
              <div className="bg-white p-8 rounded-[40px] border border-zinc-200 shadow-xl max-w-4xl mx-auto">
                <div className="flex items-center justify-between mb-8">
                  <h3 className="text-2xl font-black text-zinc-900">
                    {isEditingCompany ? "Edit Workflow" : "Company Profile"}
                  </h3>
                  <button
                    onClick={() => {
                      setIsAddingCompany(false);
                ...
                <form onSubmit={handleCompanySubmit} className="space-y-8">
                    ...
                    <div className="md:col-span-2 space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-2">
                          ...
                          <input
                            ...
                            className="w-full px-4 py-3 rounded-xl border border-zinc-200 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all font-bold"
                          />
                        </div>
                        <div className="space-y-2">
                          <label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">
                      ...
                      <div className="space-y-2">
                        ...
                        <textarea
                          ...
                          className="w-full px-4 py-3 rounded-xl border border-zinc-200 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all h-[100px] text-sm"
                        />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-6 pt-6 border-t border-zinc-100">
                    <div className="flex items-center justify-between">
                      <h4 className="text-lg font-black text-zinc-900 flex items-center gap-2 tracking-tight">
                        <Layout className="w-5 h-5 text-emerald-500" /> Pipeline
                        Builder
                      </h4>
                      <button
                        type="button"
                        onClick={addRound}
                        className="bg-zinc-100 text-zinc-900 px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-zinc-200 flex items-center gap-2"
                      >
                    ...
                    <Reorder.Group
                      ...
                      onReorder={(nw) =>
                        setCompanyForm({ ...companyForm, workflow: nw })
                      }
                      className="space-y-4"
                    >
                        ...
                        >
                          <div className="flex items-center gap-4">
                            ...
                            <select
                              ...
                              className="bg-zinc-50 px-4 py-2 rounded-xl border border-zinc-200 text-sm font-bold outline-none"
                            >
                          ...
                          {(round.type === "aptitude" ||
                            ...
                            <div className="ml-9 p-4 bg-zinc-50 rounded-2xl border border-zinc-100 flex items-center gap-4">
                              ...
                              <input
                                ...
                                className="w-16 px-2 py-1 rounded-lg border border-zinc-200 text-sm font-bold"
                              />
                                ...
                                <div className="flex flex-wrap gap-2">
                                  ...
                                  {availableTopics.length === 0 && (
                                    <p className="text-xs text-zinc-400 italic">
                                      No topics available. Import Aptitude CSV
                                      first.
                                    </p>
                                  )}
                                </div>
                          ...
                          {/* GROUP DISCUSSION SPECIFIC CONFIG (NEW) */}
                           {round.type === 'gd' && (
                             <div className="pl-9 mt-2">
                               <div className="space-y-2">
                                 <label className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">Debate Topic</label>
                                 <input 
                                   type="text" 
                                   placeholder="e.g. AI will replace junior developers within 5 years."
                                   value={round.config?.topic || ''}
                                   onChange={(e) => updateRound(round.id, { config: { ...round.config, topic: e.target.value } })}
                                   className="w-full px-4 py-2 rounded-lg border border-zinc-200 text-sm font-bold outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500"
                                 />
                               </div>
                             </div>
                           )}
                    ...
                    {companyForm.workflow.length === 0 && (
                      <div className="text-center py-16 border-2 border-dashed border-zinc-200 rounded-[32px] bg-zinc-50">
                        ...
                        <p className="text-zinc-400 font-bold text-sm">
                          Pipeline is empty. Add a stage to begin.
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  <button
                    type="submit"
                    className="w-full bg-emerald-500 text-white py-4 rounded-2xl font-black uppercase tracking-widest text-sm hover:bg-emerald-600 transition-all flex items-center justify-center gap-2 shadow-xl shadow-emerald-500/20 active:scale-95"
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {companies.map((company) => (
                  <motion.div
                    key={company.id}
                    whileHover={{ y: -4 }}
                    className="bg-white p-6 rounded-[32px] border border-zinc-200 shadow-sm flex flex-col justify-between"
                  >
                    <div>
                      ...
                      <p className="text-[10px] font-black text-emerald-600 mb-3 uppercase tracking-widest">
                        {company.targetRole}
                      </p>
                    ...
                    <div className="pt-4 border-t border-zinc-100">
                      <div className="flex items-center justify-between">
                        <span className="text-[10px] font-black text-zinc-400 uppercase tracking-widest bg-zinc-100 px-2 py-1 rounded-lg">
                          {
                            codingBank.filter((q) => q.companyId === company.id)
                              .length
                          }{" "}
                          Coding Tasks
                        </span>
          ...
          >
            <div className="bg-white rounded-[32px] border border-zinc-200 shadow-sm overflow-hidden">
              <div className="p-6 border-b border-zinc-100 flex items-center justify-between">
                ...
                <div className="flex items-center gap-4">
                  <span className="text-sm font-bold text-zinc-500 bg-zinc-100 px-4 py-2 rounded-xl">
                    {isFetchingBank ? (
                      <Loader2 className="w-4 h-4 animate-spin inline mr-2" />
                    ) : null}
                    {globalAptitudeBank.length} Questions Loaded
                  </span>
                </div>
              </div>
                ...
                {isFetchingBank ? (
                  ...
                  <>
                    {filteredAptitude.map((q) => (
                      <div
                        key={q.id}
                        className="p-6 hover:bg-zinc-50 transition-colors flex items-center justify-between group"
                      >
                        <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <button
                            onClick={() => deleteAptitudeQuestion(q.id)}
                    ...
                    {globalAptitudeBank.length === 0 && (
                      <div className="py-20 text-center">
                        ...
                        <p className="text-zinc-400 font-bold">
                          Global bank is empty. Upload a CSV to get started.
                        </p>
                      </div>
                    )}
                  </>
        ...
        {activeTab === "settings" && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -10 }}
            className="bg-white p-8 rounded-[40px] border border-zinc-200 shadow-sm max-w-2xl"
          >
            <h3 className="text-xl font-black text-zinc-900 mb-8 flex items-center gap-3">
              <MessageSquare className="text-emerald-500" /> HR Interview
              Configuration
            </h3>
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-4">
                  {(["professional", "friendly", "strict"] as const).map(
                    (tone) => (
                      <button
                        key={tone}
              ...
              <div className="p-5 bg-zinc-50 rounded-2xl border border-zinc-100 flex items-start gap-3">
                ...
                <p className="text-xs text-zinc-500 leading-relaxed font-medium">
                  <span className="font-bold text-zinc-700">System Note:</span>{" "}
                  These settings will apply to all students taking the HR
                  Interview round. The generative AI will adjust its persona and
                  visual avatar generation based on these parameters.
                </p>
              </div>
            </div>
          </motion.div>
              ...
              <div className="p-8 border-b border-zinc-100 flex items-center justify-between bg-zinc-50/50">
                <div>
                  ...
                  <p className="text-xs font-bold text-emerald-600 uppercase tracking-widest mt-1">
                    Linking to:{" "}
                    {companies.find((c) => c.id === codingForm.companyId)?.name}
                  </p>
                </div>
                <button
                  onClick={() => setIsAddingCoding(false)}
                  className="p-3 bg-white hover:bg-zinc-100 rounded-full shadow-sm border border-zinc-200 transition-colors"
                >
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-8 flex flex-col md:flex-row gap-8">
                ...
                <div className="md:w-1/2 flex flex-col border-l border-zinc-100 pl-8">
                  ...
                  <div className="flex-1 overflow-y-auto space-y-3 pr-2">
                    {codingBank
                      .filter((q) => q.companyId === codingForm.companyId)
                      .map((q) => (
                        <div
                          key={q.id}
                          className="p-4 bg-zinc-50 rounded-2xl border border-zinc-200 flex items-start justify-between group"
                        >
                          <div className="pr-4">
                          ...
                          <button
                            ...
                            className="p-2 bg-white text-zinc-400 hover:text-red-500 hover:bg-red-50 rounded-xl border border-zinc-100 transition-colors shrink-0"
                          >
                            <Trash2 size={14} />
                          </button>
                        </div>
                    ...
                    {codingBank.filter(
                      (q) => q.companyId === codingForm.companyId,
                    ).length === 0 && (
