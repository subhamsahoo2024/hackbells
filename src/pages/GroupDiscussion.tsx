...
const COMPANY_TOPICS: Record<string, string> = {
  Google:
    "Cloud computing is more of a security risk than on-premise infrastructure.",
  Microsoft: "Does a college degree still matter in the tech industry today?",
  Amazon:
    "Is the trade-off of user privacy for hyper-personalized shopping experiences ethical?",
...
export default function GroupDiscussion() {
  ...
  const [activeTypist, setActiveTypist] = useState<"none" | "Priya" | "Rahul">(
    "none",
  );
  ...
  const handleStartGD = () => {
    ...
    const msg1 = {
      role: "ai1",
      name: "Priya",
      text: `Hi everyone. The topic today is: '${debateTopic}'. I think we should start by defining the core arguments.`,
    };
    const msg2 = {
      role: "ai2",
      name: "Rahul",
      text: `Agreed, Priya. Let's hear what our third team member thinks before we dive in deep.`,
    };
  ...
  const callGroq = async (systemPrompt: string, userMessage: string) => {
    const groqApiKey = requireEnv(
      "VITE_GROQ_API_KEY_GD",
      env.groupDiscussionGroqApiKey,
    );
  ...
  const handleAction = async (userText: string | null) => {
    ...
    try {
      setActiveTypist("Priya");
      const priyaResponse = await getPersonaResponse(
        "Priya",
        "Analytical and polite",
        updatedMessages,
        !userText,
      );
      const priyaState = [
        ...updatedMessages,
        { role: "ai1", name: "Priya", text: priyaResponse } as Message,
      ];
      setMessages(priyaState);

      speak(priyaResponse, "Priya", async () => {
        setActiveTypist("Rahul");
        const rahulResponse = await getPersonaResponse(
          "Rahul",
          "Strategic and slightly challenging",
          priyaState,
          !userText,
        );
        setMessages([
          ...priyaState,
          { role: "ai2", name: "Rahul", text: rahulResponse },
        ]);
        setActiveTypist("none");
        ...
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-2">
              <span className="text-[10px] font-black uppercase text-indigo-500 tracking-widest bg-indigo-50 px-2 py-0.5 rounded-md flex items-center gap-1">
                <Building size={10} /> {selectedCompany}
              </span>
              <span className="text-[10px] font-black uppercase text-zinc-500 tracking-widest bg-zinc-100 px-2 py-0.5 rounded-md truncate max-w-[200px]">
                Topic: {debateTopic}
              </span>
            </div>
          </div>
        ...
        <div className="flex items-center gap-3">
          ...
          {hasStarted && !evaluation && (
            <button
              ...
              disabled={
                isEvaluating || activeTypist !== "none" || isBotsTalking
              }
          ...
          {!hasStarted && !evaluation ? (
            ...
            >
              ...
              <p className="text-zinc-500 mb-10 max-w-lg text-lg leading-relaxed">
                You will enter a 3-person discussion. Your topic is:
                <br />
                <strong className="text-zinc-900 text-xl block mt-4 p-4 bg-zinc-50 rounded-2xl border border-zinc-100">
                  "{debateTopic}"
                </strong>
              </p>
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-8 scrollbar-hide">
                {messages.map((msg, i) => (
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <div
                        className={`p-5 text-sm leading-relaxed shadow-sm ${
                          msg.role === "user"
                ...
                {activeTypist !== "none" && (
                  <div className="flex gap-4 max-w-[80%]">
                    <div
                      className={`w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 border text-white ${
                        activeTypist === "Priya"
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <div
                        className={`p-5 border rounded-[24px] rounded-tl-sm flex items-center gap-2 ${
                          activeTypist === "Priya"
                      ...
                      >
                        <motion.div
                          ...
                          className={`w-2 h-2 rounded-full ${activeTypist === "Priya" ? "bg-indigo-400" : "bg-orange-400"}`}
                        />
                        <motion.div
                          animate={{ opacity: [0.3, 1, 0.3] }}
                          transition={{
            ...
            >
              ...
              <h3 className="text-4xl font-black text-zinc-900 tracking-tight mb-2">
                GD Evaluation
              </h3>
              <p className="text-zinc-500 font-medium text-lg mb-10">
                Your collaborative discussion skills have been analyzed.
              </p>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-4xl">
                <div className="bg-zinc-50 p-10 rounded-[40px] border border-zinc-100 flex flex-col items-center justify-center">
                  <p className="text-[11px] font-black text-zinc-400 uppercase tracking-[0.2em] mb-4">
                    Overall Score
                  </p>
                  <div className="relative">
                    ...
                    <span className="text-2xl text-zinc-300 font-bold ml-1">
                      /100
                    </span>
                  </div>
                </div>
                ...
                <div className="bg-white p-10 rounded-[40px] border border-zinc-200 shadow-sm text-left relative overflow-hidden">
                  ...
                  <p className="text-zinc-600 leading-relaxed text-sm whitespace-pre-wrap italic">
                    "{evaluation.feedback}"
                  </p>
                  {skipCount > 0 && (
                    <p className="mt-4 text-xs font-bold text-orange-600 bg-orange-50 p-3 rounded-xl border border-orange-100">
                      Note: You passed your turn {skipCount} time(s). Passive
                      participation can impact your final score.
                    </p>
                  )}
                </div>
              </div>
