...
export default function HRInterview() {
  ...
  useEffect(() => {
    ...
    const analyzeFace = async () => {
      if (!isCameraOn || !videoRef.current || !canvasRef.current || isFinished)
        return;
      ...
      try {
        ...
        const genAI = new GoogleGenerativeAI(
          requireEnv("VITE_GEMINI_API_KEY_HR", env.hrGeminiApiKey),
        );
        const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash" });

        const prompt =
          "Analyze this candidate's posture. Reply ONLY with JSON: {'score': number 40-100, 'feedback': '3-6 word tip'}. No markdown.";
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const groqApiKey = requireEnv("VITE_GROQ_API_KEY_HR", env.hrGroqApiKey);
    ...
    return (
      <div className="h-[calc(100vh-240px)] flex flex-col items-center justify-center bg-white rounded-[40px] border border-zinc-200 shadow-sm text-center p-8">
        ...
        <p className="text-zinc-500 max-w-md mb-8">
          Select the role you are applying for. The HR AI will tailor its
          questions to match this position.
        </p>
          ...
          <div className="flex items-center gap-4 mb-8 border-b border-zinc-100 pb-6">
            ...
            <div>
              ...
              <p className="text-zinc-500">
                Here is your detailed AI performance analysis for the{" "}
                {selectedRole} role.
              </p>
            </div>
          </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-white relative">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-6 border-t border-zinc-100 bg-white shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
            <div className="max-w-5xl mx-auto flex items-end gap-3">
              <button
                onClick={toggleMic}
                className={`h-[60px] px-6 rounded-2xl flex items-center gap-2 font-bold transition-all shadow-sm border ${isMicOn ? "bg-red-50 text-red-600 border-red-200 hover:bg-red-100 animate-pulse" : "bg-zinc-100 text-zinc-600 border-zinc-200 hover:bg-zinc-200"}`}
              >
                {isMicOn ? (
        ...
        <div className="w-80 bg-zinc-50/50 p-6 overflow-y-auto border-l border-zinc-200 flex flex-col gap-6">
          ...
          <button
            ...
            className="w-full bg-zinc-900 text-white px-6 py-4 rounded-2xl font-bold hover:bg-zinc-800 transition-all flex items-center justify-center gap-2 shadow-xl shadow-zinc-900/10 disabled:opacity-50 mt-auto shrink-0"
          >
            {isSubmitting ? (
